...
import {
    FaArrowLeft, FaArrowRight, FaCheck, FaCommentDots,
    FaHeart, FaPause, FaPlay, FaPlus, FaVolumeMute, FaVolumeUp
} from "react-icons/fa";
...
const VideoCard = ({ src, profileImg, isActive }: VideoCardProps) => {
    ...
    return (
        <div className={"relative w-full h-screen"}>
            ...
            >
                ...
                {isHovered && (
                    <div className="absolute bg-black/60 inset-0" onClick={togglePlay}>
                        <motion.div
            ...
            <div className="absolute rounded-tl-4xl right-0 bottom-1 p-5 pb-10 text-white bg-gradient-to-tl from-black/50 to-transparent flex flex-col gap-7 justify-center">
                <div className="relative w-full flex items-center justify-center">
                    <img src={profileImg} alt="Profile" className="object-cover rounded-full size-16 lg:size-24 border-primary border-2" onClick={() => navigate("/profile")} />
                    <div className="absolute -bottom-2 bg-white p-1 rounded-full size-4 lg:size-6" onClick={toggleFollow}>
                        <AnimatePresence mode="wait">
                            {isFollowed ? (
                                <motion.div key="check" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }}>
                                    <FaCheck className="text-green-700" />
                                </motion.div>
                            ) : (
                                <motion.div key="plus" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }}>
                ...
                <div className="flex flex-col gap-1 items-center z-50">
                    <FaHeart className={`cursor-pointer size-5 lg:size-7 ${isLoved ? 'text-red-500' : 'text-white'}`} onClick={toggleLoved} />
