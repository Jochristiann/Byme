use uuid::Uuid;
use diesel::prelude::*;
use model::posts::{NewPost, Posts, PostFiles};
use model::schema::posts::dsl::posts as posts_dsl;
use model::schema::postfiles::dsl::postfiles as postfiles_dsl;
use model::schema::{users};
use model::schema::postfiles::postid;
...
pub async fn get_post_files_by_post_id(pool: &DbPool, ids: Uuid)->Option<Vec<PostFiles>>{
    let conn = &mut pool.get().expect("Failed to get DB connection");
    postfiles_dsl
        .filter(postid.eq(ids))
        .load::<PostFiles>(conn)
        .ok()
}
...
pub async fn insert_post_file(pool:&DbPool, post: PostFiles) -> bool{
    let conn = &mut pool.get().expect("Failed to get DB connection");

    diesel::insert_into(postfiles_dsl).values(&post).execute(conn).is_ok()
}
