...
pub async fn get_post_by_id(pool:&DbPool, id:String) ->Option<(PostResponse, String)>{
    ...
    if let Some((post, user)) = responses {
        ...
        let post_response: PostResponse = convert_post_to_post_res(post, user_response);
...
pub fn vec_convert_post_to_post_res(posts: Vec<(Posts, Users)>)->Vec<PostResponse>{
...
pub fn convert_post_to_post_res(post:Posts, user:UserResponse)->PostResponse{
    PostResponse{
        id: post.id,
        image: post.image,
        description: post.description,
